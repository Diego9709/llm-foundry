...
from llmfoundry.models.llama.modeling_llama import LlamaForCausaLlm
...
def validate_config(cfg_: DictConfig):
    """Validates compatible model and dataloader selection."""
    loaders = [cfg_.train_loader]
    if 'eval_loader' in cfg_:
        loaders.append(cfg_.eval_loader)
    for loader in loaders:
        if loader.name == 'text':
            if cfg_.model.name in ['hf_prefix_lm', 'hf_t5']:
                raise ValueError(
                    f'Model type "{cfg_.model.name}" is not supported when using the "text " ' + \
                    f'dataloader. Please use the "text_denoising" dataloader to pre-train that model type.')
        elif loader.name == 'text_denoising':
            if cfg_.model.name == 'hf_causal_lm':
                raise ValueError(
                    f'Model type "{cfg_.model.name}" is not supported when using the "text_denoising" ' + \
                    f'dataloader. Please use the "text" dataloader to pre-train that model type.')
            if loader.mixture_of_denoisers.decoder_only_format and cfg_.model.name == 'hf_t5':
                warnings.warn(
            ...
            if (not loader.mixture_of_denoisers.decoder_only_format) and cfg_.model.name == 'hf_prefix_lm':
                warnings.warn(
    ...
    if 'icl_tasks' in cfg_:
        if cfg_.model.name == 'hf_t5':
            raise ValueError(
                'ICL evaluation does not currently support Encoder-Decoder models, such as "hf_t5".'
            )

    if (cfg_.model.get('fc_type', 'torch') != 'te' and 'te' not in cfg_.model.get(
            'ffn_config', {}).get('ffn_type', 'mptmlp') and
            'fp8' in cfg_.precision):
        warnings.warn(
    ...
    if (cfg_.model.get('fc_type', 'torch') == 'te' or
            'te' in cfg_.model.get('ffn_config', {}).get('ffn_type', 'mptmlp')):
        fsdp_config = cfg_.get('fsdp_config', None)
        ...
        if fsdp_config is not None and act_ckpt == True and act_ckpt_reentrant == False:
            ...
            cfg_.fsdp_config.activation_checkpointing_reentrant = True

    if 'te' in cfg_.model.get('ffn_config', {}).get('ffn_type', 'mptmlp'):
        warnings.warn(
    ...
    if cfg_.model.get('load_in_8bit', False):
        raise ValueError(
...
def build_dataloader(cfg_: DictConfig, tokenizer: PreTrainedTokenizerBase,
    ...
    if cfg_.name == 'text':
    ...
    elif cfg_.name == 'text_denoising':
        return build_text_denoising_dataloader(
            cfg_,
            tokenizer,
            device_batch_size,
        )
    elif cfg_.name == 'finetuning':
        ...
        raise ValueError(f'Not sure how to build dataloader with config: {cfg_}')
...
def main(cfg: DictConfig) -> Trainer:
    ...
    load_ignore_keys: Optional[List[str]] = pop_config(cfg,
                                                       ...
                                                       must_exist=False,
                                                       default_value=None)
    # Enable auto-resume from model checkpoints if possible
    auto_resume_default: bool = False
    if run_name is not None \
        ...
        print('As run_name, save_folder, and save_latest_filename are set, \
                changing auto_resume default to True...')
        auto_resume_default = True
    auto_resume: bool = pop_config(cfg,
                                   'auto_resume',
                                   must_exist=False,
                                   default_value=auto_resume_default)
